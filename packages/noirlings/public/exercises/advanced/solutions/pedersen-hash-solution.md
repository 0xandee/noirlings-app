# Pedersen Hash - Solution

## Exercise ID: pedersen_hash

**Category:** hashes  
**Difficulty:** easy  
**Prerequisites:** []

## Description

Pedersen hash is a cryptographic hash function based on elliptic curve operations, widely used in zero-knowledge proofs for its deterministic properties and collision resistance.

## Solution

```noir
use std::hash::pedersen_hash;

// Basic Pedersen Hash Implementation
fn compute_pedersen_hash(x: Field, y: Field) -> Field {
    std::hash::pedersen_hash([x, y])
}

// Hash Array Implementation
fn hash_array<let N: u32>(data: [Field; N]) -> Field {
    std::hash::pedersen_hash(data)
}

// Incremental Hash Implementation
fn incremental_hash(base: Field, new_data: Field) -> Field {
    std::hash::pedersen_hash([base, new_data])
}

// This function tests if changing one input value creates a completely different hash
fn test_avalanche_effect(x: Field, y: Field, modified_x: Field) -> bool {
    let hash1 = std::hash::pedersen_hash([x,y]);
    let new_hash = std::hash::pedersen_hash([modified_x, y]);
    hash1 != new_hash
}

fn main(x: Field, y: Field, expected_hash: Field) -> pub bool {
    let hash = compute_pedersen_hash(x, y);

    // Test deterministic property - same inputs always give same output
    let hash2 = compute_pedersen_hash(x, y);
    let deterministic = hash == hash2;

    let correct_hash = hash == expected_hash;

    // Test avalanche effect with a slightly modified x value
    let modified_x = x + 1;
    let has_avalanche = test_avalanche_effect(x, y, modified_x);

    // Return true only if all tests pass
    deterministic & correct_hash & has_avalanche
}

#[test]
fn test_pedersen_hash_1() {
    let x1 = 1;
    let y1 = 2;
    let expected1 = std::hash::pedersen_hash([x1, y1]);
    assert(main(x1, y1, expected1));

    let x2 = 123456789;
    let y2 = 987654321;
    let expected2 = std::hash::pedersen_hash([x2, y2]);
    assert(main(x2, y2, expected2));
}

#[test]
fn test_string_hash() {
    let message = "Noir Gud";
    let arr = message.as_bytes().map(|charCode| charCode as Field);
    let expected_hash: Field = 0x23176ca31135ed3307b6dc2206fd3471f844d15889a8c1821b24b62049bb4832;
    let hash = std::hash::pedersen_hash(arr);
    assert(hash == expected_hash, "hash should match");
}

#[test]
fn test_array_hashing() {
    let data1 = [1, 2, 3, 4, 5];
    let hash1 = hash_array(data1);

    let data2 = [10, 20, 30];
    let hash2 = hash_array(data2);

    // Different arrays should produce different hashes
    assert(hash1 != hash2);
}

#[test]
fn test_incremental_hashing() {
    let base = 42;
    let new_data = 100;

    let incremental = incremental_hash(base, new_data);
    let direct = std::hash::pedersen_hash([base, new_data]);

    // Should be equivalent
    assert(incremental == direct);
}
```

## Key Concepts

1. **Pedersen Hash Function**: Uses elliptic curve operations for cryptographic hashing
2. **Deterministic Property**: Same inputs always produce the same hash
3. **Avalanche Effect**: Small input changes cause large output changes
4. **Array Hashing**: Can hash arrays of Field elements efficiently
5. **Incremental Hashing**: Build up hashes by combining previous results

## Documentation

- [Noir Pedersen Hash Documentation](https://noir-lang.org/docs/noir/standard_library/cryptographic_primitives/hashes#pedersen-hash)
